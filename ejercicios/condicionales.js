// 1.- Condicionales

/**
 * Ejercicio 1.1
 * 
 * Determinar si un número es par o impart
 * 
 * Crea una función llamada isPair que reciba un número como parámetro y que retorne true si es número par o false si es impar
 * 
 * isPair(number)
 */

/**
 * Ejercicio 1.2
 * 
 * Haz una función llamada greatest que reciba dos números y que regrese el mayor de los 2
 * 
 * greatest(number1, number2)
 */

/**
 * Ejercicio 1.3
 * 
 * Haz una función de nombre compareLength que reciba dos cadenas como parámetros y retorne "cadena1" si la primera cadena es más larga,  "cadena2" si la seunda cadena es más larga, o "iguales" si tienen la misma longitud
 * 
 * compareLength(string1, string2)
 */

/**
 * Ejercicio 1.4
 * 
 * Crea una función llamada getDay que recibac un número del 1 al 7 como parámetro y retorne el nombre del día de la semana correspondiente (por ejemplo, 1 = 'lunes'). Si el número no está en rango, retorne 'Número inválido'.
 * 
 * getDay(number)
 */


/**
 * Haz una función llamada gradeStudent que reciba un número del 0 al 100 como parámetro y retorne la calificación como letra: 'A' (90-100), 'B' (80-89), 'C' (70-79), 'D' (60-69), 'F' (menos de 60)
 * 
 * gradeStudent(number)
 */

// 2.- BUCLES

/**
 * Ejercicio 2.1 
 * 
 * Haz una función llamada count que reciba un número n como parámetro, y retorne una cadena que contenga todos los números enteros desde el 1 hasta n separados por comas
 */

// count(number)

/**
 * Ejercicio 2.2
 * 
 * Haz una función de nombre sumUntil que reciba un número n como parámetro y que retorne la suma de todos los números desde 1 hasta n.
 */

// sumUntil(number)

/** 
 * Ejercicio 2.3
 * 
 * Haz una funcion llamada getPairs que reciba un número n como parámetro y retorne una cadena con todos los números desde 1 hasta n que sean pares separados por coma. si puedes reutiliza la funcion isPair
*/

// getPairs(number)

// CADENAS (STRINGS)

/**
 * Ejercicio 3.1
 * 
 * Haz una función llamada vocalsCount que reciba una cadena como parámetro y regrese cuántas vocales contiene tanto mayúsculas como minúsculas
 */

// vocalsCount(string)


/** 
 * Ejercicio 3.2
 * 
 * Haz una función llamada countWords que reciba una frase como parámetro y regrese un número que indique cuántas palabras hay en la frase
 */

// countWords(phrase)

/**
 * Ejercicio 3.3
 * 
 * Haz una función llamada hideInfo que reciba una cadena como parámetro y regrese una cadena la misma cadena pero remplazando todos los caracteres por * execpto los últimos 4 dígitos. Por ejemplo, "1234567890" -> "******7890"
 */

/**
 * Ejercicio 3.4
 * 
 * Haz una función llamada getlastWord que reciba una cadena como parámetro y regrese la última palabra
 * 
 */